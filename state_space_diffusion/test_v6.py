...
def bilinear_interpolation(features: torch.Tensor, xy: torch.Tensor, grid_square_size: float, max_grid_square_inclusive: int):
    ...
    lower_ind = torch.floor((xy - 0.5) / grid_square_size).to(torch.long)
...
def sample(model, renderer, xy_image, yz_image, xz_image, start_point, device="cuda"):
    ...
    # langevin dynamics
    # first, calculate score function.
    score_xy_map, score_yz_map, score_xz_map = model(torch.stack([xy_image, yz_image, xz_image], dim=0))
    ...
    for step in range(n_steps):
        ...
        yz_loc, xz_loc, yx_loc = renderer.point_location_on_images(pos)
        grid_square_size = 16
        ...
        score_xy = bilinear_interpolation(score_xy_map, T(yx_loc), grid_square_size, max_grid_square_inclusive)
        ...
        score = torch.tensor([
            score_xy[1] + score_xz[0],
            score_xy[0] + score_yz[0],
            score_xz[1] + score_yz[1],
        ]) / 2

        # Take a step in the direction of the score
        score_pixels = score * 112.0 # 112 = 224 / 2 (224 is the image size)
        step_size = 0.1
        pos += step_size * score_pixels
...
model = VisualPlanDiffuserV6(clip).to(device).load_state_dict(torch.load("diffusion_model.pt"))

renderer = VoxelRenderer(SCENE_BOUNDS, 224, torch.tensor([0, 0, 0], device=device), device=device)

demos = get_demos()
test_examples = create_orthographic_labels(demos[0], renderer, device)
